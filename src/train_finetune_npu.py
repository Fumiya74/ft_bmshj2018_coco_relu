...
# -*- coding: utf-8 -*-
"""
Training script (bmshj2018_factorized with GDN→NPU-friendly blocks).
- Drop-in alternative to train_finetune_relu.py
- Adds CLI to control NPU replacement params (enc t/kdw/eca, dec k/gain range)
- Keeps the rest of your pipeline intact.
"""
...
def get_args():
    ...
    # NPU replacement params
    ap.add_argument("--replace_parts", type=str, default="all", choices=["encoder","decoder","all"])
...
def maybe_init_wandb(args):
    ...
    try:
        ...
        if args.resume and run_id_file.exists():
            ...
            run_id = f"{base_id}_npu_{args.replace_parts}"
            run_id_file.write_text(run_id)
            resume_mode = None
            print(f"[wandb] Starting new run (id={run_id})")
        wandb.init(project=os.environ.get("WANDB_PROJECT", "bmshj2018_npu"),
